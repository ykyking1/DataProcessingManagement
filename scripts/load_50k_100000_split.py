# -*- coding: utf-8 -*-
"""
synthetic_50k_100000.tab (50.002 sutun x 100.000 satir, 5 milyar hucre,
14,3GB) TEK INSERT'te ClickHouse'un bellek tavanina (~9,3GB) her zaman
carpiyor -- fresh restart sonrasi bile (Bolum 41.2'nin orijinal
bulgusuyla ayni).

ONEMLI DUZELTME: ilk denemede LIMIT/OFFSET ile TEK buyuk (14,3GB) MinIO
objesinden iki "yari" okumayi denedim, bu da AYNI hatayla coktu --
ClickHouse'un s3() paralel okuyucusu (ParallelReadBuffer) tampon
boyutunu ALTTAKI OBJENIN TAM boyutuna gore ayarliyor, LIMIT/OFFSET satir
sinirina gore DEGIL. Yani sorguya LIMIT koymak buyuk objenin okuma
maliyetini azaltmiyor.

GERCEK cozum (Bolum 41.2'de elle uygulanmis, burada script'e donusturuldu):
KAYNAK .tab DOSYASINI FIZIKSEL olarak iki kucuk parcaya bol, her birini
AYRI sikistir, AYRI MinIO objesi olarak yukle, sonra her birini kendi
(kucuk, ~7GB) objesinden AYRI/gecici bir ClickHouse tablosuna yukle --
boylece ClickHouse'un paralel okuyucusu her seferinde sadece KUCUK
objeye gore tampon ayirir.
"""
import time
import json
import os
...
HALF = 50000  # toplam 100.000 satirin yarisi
ZSTD_LEVEL = 12
CHUNK = 64 * 1024 * 1024  # Bolum 42.2
...
def mark_this_error(error_detail):
    """Bu script kendi icinde coker/istisna alirsa conversion_manifest +
    history'yi 'error' ile kapatir -- ana pipeline'daki mark_error()'un
    ayni deseni."""
    c = pg.cursor()
    c.execute(
...
# ---- 1) KAYNAK DOSYAYI FIZIKSEL OLARAK IKIYE BOL ----
print("Kaynak dosya fiziksel olarak ikiye bolunuyor...", flush=True)
part_a_tab = f"{GRID_DIR}/synthetic_{TAG}_partA.tab"
part_b_tab = f"{GRID_DIR}/synthetic_{TAG}_partB.tab"

t0 = time.time()
with open(SRC_TAB, "r", encoding="utf-8", newline="") as fin:
    header = fin.readline()
    with open(part_a_tab, "w", encoding="utf-8", newline="") as fa:
        fa.write(header)
        for i in range(HALF):
            line = fin.readline()
            if not line:
                break
            fa.write(line)
    with open(part_b_tab, "w", encoding="utf-8", newline="") as fb:
        fb.write(header)
        while True:
            line = fin.readline()
            if not line:
...
print(f"Bolme tamamlandi: {split_time:.1f}sn, "
      f"partA={os.path.getsize(part_a_tab)/(1024**2):.1f}MB, "
      f"partB={os.path.getsize(part_b_tab)/(1024**2):.1f}MB", flush=True)
...
try:
  for half_name, src_part, table_name in [
    ("A (ilk yari)", part_a_tab, f"synthetic_{TAG}_partA"),
    ("B (ikinci yari)", part_b_tab, f"synthetic_{TAG}_partB"),
  ]:
    print(f"--- Yari {half_name} ---", flush=True)
    out_zst = src_part + ".zst"
    object_key = f"grid/{os.path.basename(src_part)}.zst"

    t0 = time.time()
    cctx = zstd.ZstdCompressor(level=ZSTD_LEVEL)
    with open(src_part, "rb") as fin, open(out_zst, "wb") as fout:
    ...
    os.remove(out_zst)
    os.remove(src_part)  # yerel .tab parcasi artik gerekmiyor
...
error_note = (
    ...
    "ClickHouse'un s3() paralel okuyucusunun tampon boyutu ALTTAKI OBJENIN TAM "
    "boyutuna gore olcekleniyor (LIMIT/OFFSET bunu azaltmiyor, ilk denemede "
    "ayni hatayla coktu) -- KAYNAK DOSYA FIZIKSEL olarak ikiye bolunup, her "
    "yari AYRI sikistirilip AYRI MinIO objesi olarak yuklenip AYRI/gecici "
    "ClickHouse tablosuna yuklendi (Bolum 41.2/44 yontemi)."
...
cur.execute(
    ...
    (
        ...
        total_load_time,
        " + ".join(f"synthetic_{TAG}_part{x}" for x in "AB"), total_disk_bytes,
...
cur.execute(
    ...
    (
        ...
        total_compress_time, total_upload_time,
        total_load_time, " + ".join(f"synthetic_{TAG}_part{x}" for x in "AB"),
        total_disk_bytes,
        "done" if match else "verification_failed", error_note,
